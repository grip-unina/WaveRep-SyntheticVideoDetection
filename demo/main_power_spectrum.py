...
if __name__ == '__main__':
    ...
    list_res_fft = list()
    for video_input in tqdm(video_names): # for on videos
        ...
        res = [fund.batch(frames) for index, frames in data_loader] # for on frames
        res = np.concatenate(res, 0)
        res_fft = np.abs(np.fft.fftn(res, axes=(0, 1, 2)))**2  # 3d FFT
        res_fft = rescale_area(rescale_area(rescale_area(res_fft, siz[0], 0), siz[1], 1), siz[2], 2) # resizing
    ...
    res2img(np.swapaxes(res_fft.mean(2),1,0), out_shape=out_shape).save(out_png%'yt')
